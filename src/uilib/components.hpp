...
public:
  ...
  void SetOnPress(Func&& Function, Args&&... Arguments) {
    m_onPress = std::bind(std::forward(Function), std::decay<Args>(Arguments)...);
  }

  template <typename Func, typename... Args>
  void SetOnRelease(Func&& Function, Args&&... Arguments) {
    m_onRelease = std::bind(std::forward(Function), std::decay<Args>(Arguments)...);
  }

  template <typename Func, typename... Args>
  void SetOnHover(Func&& Function, Args&&... Arguments) {
    m_onHover = std::bind(std::forward(Function), std::decay<Args>(Arguments)...);
  }
